...
var handler = function (req, res) {
  ...
  if(req.url.indexOf('/redirect') == 0) {
    // Extract the orig_uri param from the request URI
    ...
    client.request({
      ...
      success: function(clientRes) {
        ...
        clientRes.on('end', function() {
          ...
          sys.log('storing ' + state.user + ' ' + creds)
  ...
  redisClient.get(user, function(err, val) {
    ...
    if(val) {
      ...
      if(!origin) {
        ...
        var redirectUri = encodeURIComponent('http://localhost:3030/redirect?orig={orig_uri}')
